...
Training_data_Name = 'train.mat'
Training_labels = sio.loadmat('/%s' % (Training_data_Name))
...
model_dir = '/%s/MRI_DPSU_Net_layer_%d_group_%d_ratio_%d' % (
    args.model_dir, layer_num, group_num, cs_ratio)

log_file_name = '/%s/Log_DPSU_Net_layer_%d_group_%d_ratio_%d.txt' % (
...
for epoch_i in range(start_epoch + 1, end_epoch + 1):
    for data in rand_loader:
        ...
        output_data = "[%02d/%02d] Loss_gate1: %.4f, Loss_gate2: %.4f, Total Loss: %.5f\n" % (epoch_i, end_epoch, loss_gate1.item(), loss_gate2.item(), loss_all.item())
        print(output_data)
